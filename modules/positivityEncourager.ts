...
export const positivityEncourager: Module = {
  onMessage: async (msg) => {
    ...
    let socialCreditChange = amountOfPositivity * 40
    if (amountOfPositivity > 0) {
      console.log(`[${'POSITIVITY'.green}] Positive message detected (${amountOfPositivity})`)
      msg.channel.send(`好公民! +${socialCreditChange}社会信用`)
    } else {
      console.log(`[${'POSITIVITY'.green}] Negative message detected (${amountOfPositivity})`)
...
const getAmountOfPositivity = (sentence: string) => analyzer.getSentiment(tokenizer.tokenize(sentence))
...
const isSentenceAboutPositiveThing = (sentence: string) => {
  return new Promise((resolve, _reject) => {
    ...
    sentenceCompromise.topics().forEach(topic => {
      let positiveThing = positiveThings.find((thing) => topic.text().toLocaleLowerCase().includes(thing))
